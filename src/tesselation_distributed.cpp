...
void facet_normals(Mesh* mesh) {
    ...
    for(auto f : mesh->triangle) {
        ...
        auto fn = normalize(cross(mesh->pos[indexof(f.second, mesh->vertex_ids)]-mesh->pos[indexof(f.first, mesh->vertex_ids)], mesh->pos[indexof(f.third, mesh->vertex_ids)]-mesh->pos[indexof(f.first, mesh->vertex_ids)]));
        // add triangle
    ...
    for(auto f : mesh->quad) {
        ...
        auto nv = (int)pos.size();
        // compute face normal
        auto fn = normalize(normalize(cross(mesh->pos[indexof(f.second, mesh->vertex_ids)]-mesh->pos[indexof(f.first, mesh->vertex_ids)], mesh->pos[indexof(f.third, mesh->vertex_ids)]-mesh->pos[indexof(f.first, mesh->vertex_ids)])) +
                            normalize(cross(mesh->pos[indexof(f.third, mesh->vertex_ids)]-mesh->pos[indexof(f.first, mesh->vertex_ids)], mesh->pos[indexof(f.fourth, mesh->vertex_ids)]-mesh->pos[indexof(f.first, mesh->vertex_ids)])));
        // add quad
