...
static jsonvalue _to_jsonvalue(const picojson::value& pjson) {
    ...
    else if(pjson.is<int64_t>())
        return jsonvalue( pjson.get<int64_t>() );
    else if(pjson.is<double>())
        return jsonvalue( pjson.get<double>() );
...
jsonvalue parse_cmdline(const vector<string>& args, const CommandLine& cmd) {
    ...
    for(auto arg : cmd.arguments) {
        if(largs.empty()) {
            ...
            auto asd = _cmdline_parse_value(largs[0], arg, cmd);
            parsed[arg.name] = asd;
