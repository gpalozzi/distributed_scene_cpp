...
static timestamp_t get_timestamp(){
    ...
    return  now.tv_usec + (timestamp_t)now.tv_sec * 10000000;
}
...
struct vec2id {
    ...
    // Element access
    timestamp_t& operator[](int i) { return (&first)[i]; }
    // Element access
...
inline ostream& operator<<(ostream& os, const vec3f& v) { return os << '[' << v.x << ',' << v.y << ','<< v.z << "]\n"; }
