...
struct jsonvalue {
    ...
    enum _Type { nullt, boolt, doublet, longt, stringt, arrayt, objectt };
    _Type _type = nullt;    // current type
    union {
        bool    _b; // bool value
        double  _d; // number value
        int64_t  _i; // number value
    ...
    explicit jsonvalue(int64_t i) : _type(longt), _i(i) { }
    ...
    bool is_id() const { return _type == longt; }
    ...
    int as_int() const { error_if_not(is_number() || is_id(), "wrong type"); return (is_number())?(int)_d:(int)_i; }
    ...
    timestamp_t as_id() const { error_if_not(is_id(), "wrong type"); return (timestamp_t)_i; }
